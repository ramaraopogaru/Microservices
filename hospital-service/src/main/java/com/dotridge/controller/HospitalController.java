...
public class HospitalController 
{
	...
	public String viewAllHospitals(Model model)
	{
		try
		{
			...
			int recordsPerPage = Math.round((uiListSize / 5) + 1);
	...
	public ResponseEntity<HospitalBean> addHospital(@RequestBody HospitalBean hospitalBean)
	{
		System.out.println(hospitalBean.toString());
		HospitalBean hospitalBean2 = hospitalService.createHospital(hospitalBean);
		return new ResponseEntity<HospitalBean>(hospitalBean2, HttpStatus.OK);
		
	}

	
	@RequestMapping(value="/getHospitalByName/{hospName}")
	public ResponseEntity<HospitalBean> addHospital(@PathVariable("hospName") String hospName)
	{
		System.out.println(hospName);
		HospitalBean hospitalBean2 = hospitalService.getHospitalByName(hospName);
		return new ResponseEntity<HospitalBean>(hospitalBean2, HttpStatus.OK);
